...
Integrated carousel tester v0.10 - self-supported central boss correction
...
- Preserve visibly different lower ring heights by replacing the full low top
  cap with local upper support pads/spokes.
- Make the visible central coupling boss emerge from the center/index ring
  reference height.
- Add a central printable support collar so the central boss is not a floating
  or long-bridge feature during FDM printing.
...
upper_support_z = ring_2_height - 0.1; // slight overlap with the center/index ring for printable union
...
upper_support_pad_diameter = holder_outer_diameter + 4;
upper_support_spoke_width = 5;
upper_support_hub_diameter = 22;
upper_support_spoke_length = upper_holder_pitch_radius + upper_support_pad_diameter / 2;
...
module upper_platform_disc() {
    if (show_integrated_top_cap) {
        if (preserve_stepped_ring_heights) {
            /*
            v0.9: avoid the former full low circular cap that visually leveled
            the 9/14/10 mm lower rings. Local pads and radial spokes provide
            an upper printable connection while preserving open/stepped lower
            indexing geometry.
            */
            translate([0, 0, upper_support_z])
                cylinder(h = top_cap_height, d = upper_support_hub_diameter);

            for (i = [0 : len(position_angles) - 1]) {
                rotate([0, 0, position_angles[i]]) {
                    translate([upper_support_spoke_length / 2, -upper_support_spoke_width / 2, upper_support_z])
                        cube([upper_support_spoke_length, upper_support_spoke_width, top_cap_height]);
                    translate([upper_holder_pitch_radius, 0, upper_support_z])
                        cylinder(h = top_cap_height, d = upper_support_pad_diameter);
                }
            }
        } else {
            // Legacy full cap mode retained only for comparison with v0.8.
            translate([0, 0, lower_ring_reveal_height])
                cylinder(h = integrated_platform_height - lower_ring_reveal_height, d = upper_disc_diameter);
        }
    }
}

module central_print_support_collar() {
    /*
    v0.10: printable central collar. This is added because v0.8/v0.9 could
    leave the central boss as a late-starting/floating feature in the slicer.
    The collar gives the center feature material continuity from the build
    plate/base region up to the center/index ring reference height. The screw
    and conical receiver clearances are subtracted later.
    */
    if (show_central_print_support_collar) {
        translate([0, 0, central_print_support_collar_z_min])
            cylinder(
                h = central_print_support_collar_height,
                d = central_print_support_collar_diameter
            );
    }
}
...
module center_coupling_projection() {
    ...
    v0.9 makes the *visible solid boss* emerge from the center/index ring
    reference height, while the conical internal receiver clearance remains
    dimensioned over the full central path for screw/mechanism review.
    */
    if (show_center_coupling_projection) {
        translate([0, 0, central_visible_boss_z_min])
            cylinder(
                h = central_visible_boss_height,
