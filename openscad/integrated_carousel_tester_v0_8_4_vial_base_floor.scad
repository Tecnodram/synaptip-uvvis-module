...
Integrated carousel tester v0.8.4 - vial-base full floor only
...
- Treat the vial/cuvette base plane as the mechanical zero/joint plane: vial
  holders project upward from this plane and lower indexing rings project
  downward from it with their corresponding heights.
- Add a complete thin floor only at the vial/cuvette base plane, not a deep cap
  that fills the lower indexing circles.
...
vial_base_joint_z = integrated_platform_height; // mechanical zero/joint plane in print coordinates
...
central_ring_reference_z = vial_base_joint_z; // top of center/index lamina at the vial-base joint plane
vial_base_reference_z = vial_base_joint_z; // plane where vial holders begin / mechanical joint
...
module annular_rib_down_from(top_z, outer_diameter, rib_width, rib_height) {
    /*
    v0.8.3 convention: the vial-base plane is the shared mechanical joint.
    Lower indexing rings are defined by how far they project downward from
    that plane, preserving the measured/reference heights.
    */
    translate([0, 0, top_z - rib_height])
        annular_rib(outer_diameter, rib_width, rib_height);
}
...
module lower_indexing_features() {
    if (show_lower_indexing_rings) {
        // Visible lower indexing geometry projected downward from the vial-base joint plane.
...
module upper_platform_disc() {
    if (show_integrated_top_cap) {
        if (show_vial_base_joint_floor) {
            /*
            v0.8.4: complete floor only at the vial/cuvette base plane.
            This is the shared joint where the upper holders begin and the
            lower indexing rings terminate upward. It is intentionally thin and
            does not extend downward as a full cap over the indexing circles.
            */
            translate([0, 0, vial_base_joint_floor_z])
                cylinder(h = vial_base_joint_floor_thickness, d = upper_disc_diameter);
        }
...
module vial_holder_pattern() {
    for (i = [0 : len(position_angles) - 1]) {
        rotate([0, 0, position_angles[i]])
            translate([upper_holder_pitch_radius, 0, vial_base_joint_z])
                vial_holder();
    }
}
